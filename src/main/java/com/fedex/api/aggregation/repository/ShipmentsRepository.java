...
public class ShipmentsRepository {
    ...
    private final ParameterQueue shipmentsQueue;
    private final ShipmentsApiClient shipmentsApiClient;

    public void enqueue(@NonNull List<String> orderNumbers) {
        for (String orderNumber : orderNumbers) {
            shipmentsQueue.enqueue(orderNumber);
        }
    }

    @Async
    public CompletableFuture<Map<String, List<String>>> retrieveShipments() {
        List<String> orderNumbers = getOrderNumbers();

        if (CollectionUtils.isEmpty(orderNumbers)) {
            return CompletableFuture.completedFuture(null);
        }
        return CompletableFuture.completedFuture(shipmentsApiClient.retrieveShipments(orderNumbers));
    }

    protected List<String> getOrderNumbers() {
        List<String> orderNumbers = shipmentsQueue.getParameters();
        log.info("Retrieve shipments for: {}", StringUtils.join(orderNumbers, ","));
